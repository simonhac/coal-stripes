...
- In `useGestureSpring`, every new gesture or navigation seeds from the one function `currentVisualDays()` — never from a raw ref and never from the parent's `currentOffset` directly. The parent's offset is authoritative only at rest *and* once it has caught up: it commits on a transition lane, so for a few frames after a settle (and throughout any glide) it is days behind, and seeding from it there snaps the view backwards.

- A touch must arrest an in-flight spring at **pointerdown**, which for @use-gesture means inside the `from` callback — not in the handler's `first` block. `first` never fires at all for a filtered tap (@use-gesture bails out of `compute()` below `tapsThreshold`, so the handler is called exactly once, on pointerup, with `tap: true` and `first: false`), and even for a real drag it arrives only after the finger has crossed 3px, by which time the glide has travelled past the seed. Corollary: on the tap path nothing has written the per-gesture refs, so read the handler's own `offset`, not `lastActivePRef`.
