 ...
 */
export function usePrefetchAdjacentYears(startYear: number | null, endYear: number | null): void {
  ...
  useEffect(() => {
    ...
    for (const year of [startYear - 2, startYear - 1, endYear + 1, endYear + 2]) {
      if (isValidYear(year)) {
        void queryClient.prefetchQuery(yearQueryOptions(year));
      }
    }
  }, [queryClient, startYear, endYear]);
}
