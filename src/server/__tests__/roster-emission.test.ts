/**
 * Behavioural tests for what the server puts in a year's payload — the single
 * roster, its per-unit status, and per-network error tolerance. The fleet VIEWS
 * derived from that payload are tested in @/shared/__tests__/fleet-filter.
...
describe('roster emission', () => {
  it('asks OpenElectricity for operating AND retired units, and tags each with its status', async () => {
    getFacilities.mockResolvedValue({
      table: {
        getRecords: () => [
          unitRecord({ facility_code: 'LIVE', facility_name: 'Live', unit_code: 'LV01' }),
          unitRecord({
            facility_code: 'GONE',
            facility_name: 'Gone',
            unit_code: 'GN01',
            unit_status: 'retired',
            unit_last_seen: `${YEAR}-06-30T08:00:00+10:00`,
          }),
        ],
      },
    });
    getFacilityData.mockImplementation((_network: string, codes: string[]) =>
      Promise.resolve({
        datatable: { getRows: () => (codes.includes('LIVE') ? yearRows('LV01', 'NEM') : []) },
      })
    );
    ...
    expect(getFacilities).toHaveBeenCalledWith(
  ...
  it('emits an all-null row for a roster unit with no data this year', async () => {
    // The client's `current` view recognises these rows by their all-null
    // history and drops them; see @/shared/fleet-filter. That only works
    // because they are emitted unconditionally here.
    getFacilities.mockResolvedValue({
      table: {
        getRecords: () => [
    ...
    const duids = result.data.map((u) => u.duid);
    expect(duids).toContain('WD01');
    expect(duids).toContain('ND01');

    const nd = result.data.find((u) => u.duid === 'ND01')!;
    expect(nd.history.data.every((v) => v === null)).toBe(true);
    const wd = result.data.find((u) => u.duid === 'WD01')!;
  ...
  it('propagates a non-NoDataFound upstream error rather than swallowing it', async () => {
    ...
    const service = new CapFacDataService('key');
    await expect(service.getCapacityFactors(YEAR)).rejects.toThrow();
  }, 10000);
});
...
describe('retired-unit colouring (fill precedence)', () => {
  ...
  it('never paints a retired unit red into the future — future days are null, not 0', async () => {
    ...
    const service = new CapFacDataService('key');
    const full = await service.getCapacityFactors(futureYear);
    const data = full.data.find((u) => u.duid === 'LD01')!.history.data;
